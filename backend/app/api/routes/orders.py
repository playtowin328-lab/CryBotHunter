...
async def reconcile_orders(_: User = Depends(current_user), db: AsyncSession = Depends(get_db)) -> dict[str, int]:
    return await OrderReconciliationService().reconcile(db)
